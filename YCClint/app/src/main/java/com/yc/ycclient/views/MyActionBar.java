...
import com.yc.ycclient.utils.ScreenUtil;
...
public class MyActionBar extends RelativeLayout {
    ...
    private void setAttrs(Context context, TypedArray ta) {
        ...
        if (titleSize != 0) {
            mTitleTextView.setTextSize(ScreenUtil.getFontSize(titleSize / 2.3f));// 2.3f 是根据测试计算出来的
        }
        ...
        if (leftSize != 0) {
            mLeftTextView.setTextSize(ScreenUtil.getFontSize(leftSize / 2.3f));
        }
        ...
        if (rightSize != 0) {
            mRightTextView.setTextSize(ScreenUtil.getFontSize(rightSize / 2.3f));
        }
